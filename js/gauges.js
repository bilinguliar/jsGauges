var jsGauges = {
    ...
    getData: function () { // Loading JSON
        ...
        jsGauges.settings.interval = 10000; // AJAX refreshing interval in ms.
        // SETTINGS ENDED

        xmlHttp.onreadystatechange = function () {

            if (xmlHttp.readyState == 4) {      // If downloading finished

                if (xmlHttp.status == 200) {    // without errors

                    if (!jsGauges.json) {       // If data loaded first time - build gauges.

                        jsGauges.json = JSON.parse(xmlHttp.responseText);
                        jsGauges.placeDivs();
        ...
        xmlHttp.open("GET", "jsgauges.json" + '?r=' + Math.random(), true); // Avoid browser cashing
    ...
    update: function () {
        ...
        function setColumnColor(settings, value) {
            ...
            if (value >= settings.greenValueStart && value <= settings.greenValueEnd) {

                return settings.columnGreenColor;
            } else if (value >= settings.yellowValueStart && value <= settings.yellowValueEnd) {

                return settings.columnYellowColor;
            } else if (value >= settings.redValueStart && value <= settings.redValueEnd) {

                return settings.columnRedColor;
            } else {
                alert("Current linear gauge value do not match any color zone");
                return "pink"; // Just to turn attention
            }
        }
